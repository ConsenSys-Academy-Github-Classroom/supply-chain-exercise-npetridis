...
contract SupplyChain {
    ...
    modifier forSale(uint256 _sku) {
        require(items[_sku].seller != address(0), "Item does not exist"); // verify the sku struct field is correct
        require(items[_sku].state == State.ForSale, "Item is not for sale");
        _;
    }

    modifier sold(uint256 _sku) {
        require(items[_sku].price > 0, "Item does not exist"); // verify the sku struct field is correct
    ...
    constructor() public {
        ...
        owner = msg.sender;
        // 2. Initialize the sku count to 0. Question, is this necessary? -> NO, 0 is its default value
        skuCount = 0;
    }
